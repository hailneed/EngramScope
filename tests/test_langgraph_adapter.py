...
def test_langgraph_store_emits_events_without_raw_payloads(tmp_path):
    ...
    results = store.search(("memories", "alice"), query="dark mode", limit=5)
    ...
    assert recall.input_summary["query_chars"] == len("dark mode")
    assert recall.output_summary["result_count"] == 1
    assert write.namespace == "memories/alice"
    assert write.input_summary["value_keys"] == ["secret", "text"]
