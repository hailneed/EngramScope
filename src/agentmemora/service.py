...
from .models import ConflictEvent, MemoryCreate, MemoryRecord, OperationEvent, OperationEventCreate, RecallHit
from .store import SQLiteStore


class AgentMemora:
    """Framework-agnostic observability and reference memory service."""

    def __init__(self, db_path: str | Path = "agentmemora.db") -> None:
    ...
    def events(self, limit: int = 100, provider: str | None = None, operation: str | None = None) -> list[OperationEvent]:
        return self.store.list_events(limit=limit, provider=provider, operation=operation)
    ...
    def remember(self, memory: MemoryCreate | None = None, **kwargs) -> tuple[MemoryRecord, ConflictEvent | None]:
        ...
        self.record(provider="agentmemora", operation="write", namespace=record.namespace, key=record.key, memory_ids=[record.id], input_summary={"memory_type": record.memory_type, "source": record.source}, output_summary={"status": record.status, "conflict": bool(supersedes)})
        if previous and supersedes:
            ...
            self.record(provider="agentmemora", operation="conflict", namespace=record.namespace, key=record.key, memory_ids=[previous.id, record.id], input_summary={"previous": previous.value, "current": record.value}, output_summary={"resolution": "previous_marked_historical"})
    ...
    def recall(self, query: str, namespace: str = "default", limit: int = 5) -> list[RecallHit]:
        ...
        self.record(provider="agentmemora", operation="recall", namespace=namespace, query=query, memory_ids=[hit.memory.id for hit in hits], input_summary={"limit": limit}, output_summary={"result_count": len(hits), "scores": [hit.score for hit in hits]})
        return hits
