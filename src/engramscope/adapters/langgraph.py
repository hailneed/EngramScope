...
class ObservedLangGraphStore:
    """Proxy a LangGraph BaseStore and emit provider-neutral EngramScope events.

    The adapter deliberately avoids importing LangGraph so EngramScope stays usable
    without optional framework dependencies. Pass any BaseStore-compatible object.
    Raw values and search queries are not persisted unless ``capture_payloads=True``.
    """
        ...
        *,
        ...
        self.lens.record(
            OperationEventCreate(
                provider="langgraph",
                operation=operation,
                namespace="/".join(str(part) for part in namespace),
                key=key,
                query=query,
